...
def validate_semantic_submission(runner: PipelineRunner, stage: str, payload: dict[str, Any]) -> None:
    if stage == "footage":
        _validate_footage(runner, payload)
    elif stage == "match":
        _validate_match(runner, payload)


def _validate_footage(runner: PipelineRunner, payload: dict[str, Any]) -> None:
    scaffold = runner.artifacts.read("footage_profiles_scaffold")
    expected = {str(c["clip_id"]): c for c in scaffold.get("clips") or []}
    ...
    for clip_id, spec in actual.items():
        ...
        for index, segment in enumerate(segments, 1):
            ...
            if not any(float(start)-1e-6 <= t <= float(end)+1e-6 for t in evidence_times): raise ValueError(f"{clip_id} segment {index} requires in-range frame evidence")
            previous = float(end)


def _validate_match(runner: PipelineRunner, payload: dict[str, Any]) -> None:
    blueprint = runner.artifacts.read("reference_blueprint")
    profiles = runner.artifacts.read("footage_profiles")
    reference_ids = [str(x["id"]) for x in blueprint.get("segments") or []]
    footage_ids = {str(s["id"]) for c in profiles.get("clips") or [] if c.get("usable", True) for s in c.get("segments") or []}
    ...
    if set(by_ref) != set(reference_ids) or len(matches) != len(reference_ids): raise ValueError("match proposal must cover every reference segment exactly once")
    improvements = {str(x.get("reference_segment_id") or "") for x in payload.get("improvement_requests") or [] if isinstance(x, dict)}
    for ref in reference_ids:
        item = by_ref[ref]; footage_id = str(item.get("footage_segment_id") or "")
        if footage_id not in footage_ids: raise ValueError(f"{ref} selects unknown/unusable footage segment {footage_id!r}")
