...
class FootageSemanticProgress:
    ...
    def activate(self) -> dict[str, Any] | None:
        ...
        if active_id:
            ...
            if active.get("status") in {"pending", "waiting_agent"}:
                return active
    ...
    def replayed_refinement(
        ...
        if batch is None or batch.get("status") != "pending" or not batch.get("blocked_by"):
            return None
    ...
    def _validate_partial(
        ...
        if payload.get("batch_id") not in {None, batch["batch_id"]}:
