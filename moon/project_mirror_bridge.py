...
class ProjectMirrorBridge:
    ...
    def publish(self, stage: str) -> dict[str, Any]:
        ...
        task = self.exchange.create(
            ...
            revision=revision,
            manifest=manifest,
            result_schema=self._result_schema(handoff["output_contract"]),
            inputs=inputs,
        )
        if batch:
        ...
        request = {
            **task,
            ...
            "route": {"revision": task["revision"], "batch": inputs.get("batch")},
        }
        return {
            "status": "WAITING_AGENT",
            "idempotent": task["task_id"] in preexisting_ids,
            "request": request,
            "remote": {"transport": "local_sync", "remote_path": str(self.transport.root), "request_path": f"tasks/{task['task_id']}/request.json"},
        }

    def poll_once(self) -> dict[str, Any] | None:
        current = self.transport.read_json("tasks/current.json")
        if not current:
            return None
        task = self.exchange.store.read(str(current["task_id"]))
        if not task:
            return None
        manifest = self.transport.read_json("manifest.json") or {}
        result = self.exchange.poll(task, manifest)
        ...
        if response.get("status") == "needs_refinement":
            requests = (response.get("result") or {}).get("requests") or []
            refinement = self._create_refinement(task, manifest, requests)
        ...
        def consume(accepted: dict[str, Any]) -> dict[str, Any]:
            ...
            batch = progress.batch_for_request(str(task["task_id"])) if progress else None
            if batch:
                outcome = progress.complete(str(task["task_id"]), payload, result["receipt"]["response_sha256"])
    ...
    def _create_refinement(self, task: dict[str, Any], manifest: dict[str, Any], requests: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def _public_task(task: dict[str, Any]) -> dict[str, Any]:
        return {key: value for key, value in task.items() if key not in {"project_root", "evidence_root", "path"}}
    ...
    def _asset_scope(manifest: dict[str, Any], stage: str) -> list[dict[str, Any]]:
        ...
        return [
            {"asset_id": item["asset_id"], "content_sha256": item["content_sha256"], "proxy_path": item["proxy_path"]}
            for item in manifest.get("assets", [])
            if item.get("role") in roles
        ]

    @staticmethod
    def _result_schema(contract: dict[str, Any]) -> dict[str, Any]:
        if contract.get("type") == "object" and "properties" in contract:
            return contract
        return {"type": "object"}
