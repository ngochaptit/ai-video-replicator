...
5. Confirm installation and choose the initialized Moon project directory when
   prompted. The development default is `D:\AI EDIT VIDEO\8.26`.
...
`MOON_PROJECT_ROOT` remains available as a fallback for direct invocation.
...
All diagnostics are written to stderr. Standard output is reserved for MCP
JSON-RPC messages from `moon.mcp_adapter`.
